  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        // Scan all sheets to find the one with 'Customer Name'
        for (const sheetName of workbook.SheetNames) {
          const worksheet = workbook.Sheets[sheetName];
          // Read only the first 20 rows to find the header
          const sheetData: any[][] = XLSX.utils.sheet_to_json(worksheet, { header: 1, range: 0, defval: null });

          // Check first 20 rows for 'Customer Name'
          const foundRowIndex = sheetData.slice(0, 20).findIndex(row =>
            row && row.some((cell: any) => String(cell).trim().toLowerCase() === 'customer name')
          );
        ...
        if (!targetSheetName) {
          setSummary({ creations: 0, errors: [`Could not find 'Customer Name' column in any sheet (checked first 20 rows of each).`] });
          return;
        }
        ...
        for (const [index, row] of jsonData.entries()) {
          const customerName = getValue(row, 'Customer Name') ? String(getValue(row, 'Customer Name')) : null;
          const originalId = getValue(row, 'Original Invoice ID') ? String(getValue(row, 'Original Invoice ID')) : null;
          const date = getValue(row, 'Invoice Date'); // Can be string, number (Excel date)
          const total = getValue(row, 'Total Amount') ? Number(getValue(row, 'Total Amount')) : null;
          ...
          invoicesToCreate.push({
            ...
            invoiceDate,
            totalAmount: total,
            amountAlreadyPaid: Number(getValue(row, 'Amount Already Paid') || 0),
          });
        }
