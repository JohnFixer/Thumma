  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (!targetSheetName) {
          setSummary({ billsToCreate: 0, newSuppliers: 0, errors: [`Could not find 'Supplier Name' column in any sheet (checked first 20 rows of each).`] });
          return;
        }
        ...
        for (const [index, row] of jsonData.entries()) {
          ...
          const supplierName = getValue(row, 'Supplier Name') ? String(getValue(row, 'Supplier Name')).trim() : null;
          const amount = getValue(row, 'Amount') ? Number(getValue(row, 'Amount')) : null;
          ...
          billsToImport.push({
            supplierName,
            invoiceNumber: String(getValue(row, 'Invoice Number') || `IMPORT-${Date.now()}-${rowNum}`),
            billDate: handleExcelDate(getValue(row, 'Bill Date')),
            dueDate: handleExcelDate(getValue(row, 'Due Date')),
            amount,
            notes: getValue(row, 'Notes') ? String(getValue(row, 'Notes')) : undefined,
            isNewSupplier: isNew,
            rowNum,
          });
        }
