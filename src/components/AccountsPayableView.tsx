...
const AccountsPayableView: React.FC<AccountsPayableViewProps> = ({ bills, suppliers, currentUser, onAddBillClick, onImportBillsClick, onEditBillClick, onPayBillClick, onDeleteBill, t, language }) => {
    ...
    const [sortField, setSortField] = useState<'supplier' | 'invoice' | 'dueDate' | 'amount' | 'balance' | 'status'>('dueDate');
    ...
    const filteredBills = useMemo(() => {
        ...
        return enrichedBills.filter(bill => {
            ...
            switch (sortField) {
                case 'supplier':
                    comparison = (supplierMap.get(a.supplierId) || '').localeCompare(supplierMap.get(b.supplierId) || '');
                    break;
                case 'invoice':
                ...
                case 'amount':
                    ...
                    break;
                case 'balance':
                    comparison = (a.amount - a.paidAmount) - (b.amount - b.paidAmount);
                    break;
                case 'status':
    ...
    }, [enrichedBills, searchQuery, statusFilter, supplierMap, cardFilter, sortField, sortDirection]);
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                            <tr>
                                <th scope="col" className="px-6 py-3 cursor-pointer hover:bg-gray-100" onClick={() => handleSort('supplier')}>
                                    {t('suppliers')} <SortIcon field="supplier" />
                                </th>
                                <th scope="col" className="px-6 py-3 cursor-pointer hover:bg-gray-100" onClick={() => handleSort('invoice')}>
                                    {t('invoice_number')} <SortIcon field="invoice" />
                                </th>
                                <th scope="col" className="px-6 py-3 cursor-pointer hover:bg-gray-100" onClick={() => handleSort('dueDate')}>
                                    {t('due_date')} <SortIcon field="dueDate" />
                                </th>
                                <th scope="col" className="px-6 py-3 text-right cursor-pointer hover:bg-gray-100" onClick={() => handleSort('amount')}>
                                    {t('amount')} <SortIcon field="amount" />
                                </th>
                                <th scope="col" className="px-6 py-3 text-right cursor-pointer hover:bg-gray-100" onClick={() => handleSort('balance')}>
                                    {t('balance_due')} <SortIcon field="balance" />
                                </th>
                                <th scope="col" className="px-6 py-3 text-center cursor-pointer hover:bg-gray-100" onClick={() => handleSort('status')}>
