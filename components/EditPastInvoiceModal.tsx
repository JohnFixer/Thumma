...
import type { PastInvoiceData, Customer, Transaction } from '../types';
import { XMarkIcon, ArrowUpTrayIcon, ClipboardDocumentListIcon } from './icons/HeroIcons';
import type { TranslationKey } from '../translations';

interface EditPastInvoiceModalProps {
  isOpen: boolean;
  onClose: () => void;
  onEdit: (invoiceId: string, data: PastInvoiceData) => void;
...
const EditPastInvoiceModal: React.FC<EditPastInvoiceModalProps> = ({ isOpen, onClose, onEdit, invoice, customers, showAlert, t }) => {
  ...
  useEffect(() => {
    if (isOpen && invoice) {
      ...
      setOriginalInvoiceId(invoice.items[0]?.variantId || '');
      setInvoiceDate(new Date(invoice.date).toISOString().split('T')[0]);
      setTotalAmount(String(invoice.total));
      setAmountAlreadyPaid(String(invoice.paid_amount));
      setFile(null);
      setExistingFileUrl(invoice.file_url || null);
    }
  }, [isOpen, invoice]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center" aria-modal="true" role="dialog" onClick={onClose}>
      <div className="bg-surface rounded-lg shadow-xl w-full max-w-lg m-4" onClick={e => e.stopPropagation()}>
        <div className="p-4 border-b flex justify-between items-center">
          <h3 className="text-lg font-semibold text-text-primary">Edit Past Invoice</h3>
              ...
              <div {...getRootProps()} className={`mt-1 flex justify-center items-center w-full px-6 py-6 border-2 border-gray-300 border-dashed rounded-md cursor-pointer transition-colors ${isDragActive ? 'bg-blue-50 border-primary' : 'bg-background hover:bg-gray-100'}`}>
                ...
                <div className="text-center">
                  ...
                  {file ? (
                    ...
                    <>
                      <p className="mt-2 text-sm text-text-secondary"><span className="font-semibold text-primary">Click to upload</span> or drag and drop to replace</p>
