    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center no-print" aria-modal="true" role="dialog" onClick={onClose}>
      <div className="bg-surface rounded-lg shadow-xl w-full max-w-sm m-4" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-2 max-h-[60vh] overflow-y-auto">
          <div id="receipt-to-print" className="bg-white text-black p-4 w-full mx-auto text-sm leading-relaxed">
            ...
            <table className="w-full text-sm font-sans">
              <thead>
                <tr className="border-b border-dashed border-black">
                  <th className="text-left font-bold py-1">{t_receipt('item')}</th>
                  <th className="text-center font-bold py-1">{t_receipt('qty')}</th>
                  <th className="text-right font-bold py-1">{t_receipt('price')}</th>
                  <th className="text-right font-bold py-1">{t_receipt('total')}</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={item.variantId}>
                      <td className="align-top py-1">
                        {item.name[receiptLanguage]} ({item.size})
                      </td>
                      <td className="text-center align-top py-1 font-mono">{item.quantity}</td>
                      <td className="text-right align-top py-1 font-mono">{price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                      <td className="text-right align-top py-1 font-mono">{(item.quantity * price).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                    </tr>
                  )
                })}
              </tbody>
            ...
            <div className="border-t border-dashed border-black mt-2 pt-2 text-sm">
              <div className="flex justify-between font-sans">
                <span>{t_receipt('subtotal')}:</span>
                <span className="font-mono">{transaction.subtotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              </div>
              <div className="flex justify-between font-sans">
                <span>{t_receipt('tax_7')}:</span>
                <span className="font-mono">{transaction.tax.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              </div>
              {transaction.transportationFee && transaction.transportationFee > 0 && (
                  <div className="flex justify-between font-sans">
                    <span>{t_receipt('transportation_fee')}:</span>
                    <span className="font-mono">{transaction.transportationFee.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
              )}
               {transaction.appliedStoreCredit && (
                  <div className="flex justify-between font-bold font-sans">
                    <span>{t_receipt('store_credit')}:</span>
                    <span className="font-mono">- {transaction.appliedStoreCredit.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
                )}
              <div className="flex justify-between font-bold text-lg mt-1 font-sans">
                <span>{t_receipt('total').toUpperCase()}:</span>
                <span className="font-mono">{transaction.total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              </div>
            </div>
            <div className="border-t border-dashed border-black mt-2 pt-2 text-sm font-sans">
                <p>{t_receipt('payment_method')}: {getTranslatedPaymentMethod(transaction.paymentMethod)}</p>
            </div>
            <div className="text-center mt-4 text-xs font-sans">
