...
const AddProductModal: React.FC<AddProductModalProps> = ({ isOpen, onClose, onAddProduct, initialBarcode, showAlert, t }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.en || !name.th || !selectedMainCategory || !selectedSubCategory) {
    ...
    onAddProduct({ name, description, category: `${selectedMainCategory}.${selectedSubCategory}`, imageUrl: imagePreview || undefined, variants: variantsWithNumbers });
  };
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-text-secondary">{t('product')} Image (Optional)</label>
