...
const App: React.FC = () => {
    ...
    const handleLogin = (username: string, password: string) => { 
        ...
        const permissions = getPermissionsFromRoles([Role.ADMIN]); // Simulate admin login for now
        ...
        setActiveView(loggedInUser.settings?.defaultLoginView || 'dashboard');
        setNavigationHistory([loggedInUser.settings?.defaultLoginView || 'dashboard']);
    };
    const handleLogout = () => { 
        setCurrentUser(null); 
    ...
    const handleStartSimulation = (role: Role) => {
        ...
        const simulatedUser = { ...currentUser, role: [role], permissions: simulatedPermissions };
    ...
    const handleBack = () => {
        ...
        historyCopy.pop(); // Remove current view
        const previousView = historyCopy[historyCopy.length - 1];
    ...
    return (
        <div className="flex h-screen bg-background">
            {currentUser && !currentUser.role.includes(Role.CEO) && (
                <Sidebar
            ...
            {/* MODALS */}
            <AddProductModal isOpen={isAddProductModalOpen} onClose={() => {setIsAddProductModalOpen(false); setInitialBarcodeForAdd(null);}} onAddProduct={handleAddProduct} initialBarcode={initialBarcodeForAdd} showAlert={showAlert} t={t} />
            ...
            <ImportProductsModal isOpen={isImportModalOpen} onClose={() => setIsImportModalOpen(false)} onApplyImport={() => {}} products={products} t={t} />
